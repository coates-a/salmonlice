#' Salmon lice population model
#' 
#' Gives the change in number of lice (of different genotypes and life stages) across a connected farm network over time

#' @param ts Number of time steps in simulation
#' @param z Number of lice groups per farm (number stages * number genotypes)
#' @param size Number of lice groups over all farms (z * number farms = number of rows in N)
#' @param years.start Number of spin-up years (Change from 1st (starting) to 2nd distribution after t > years.start
#'
#' @param N Louse population matrix. Number of lice per farm, stage and genotype (rows), and per time-step (columns)
#' @param l Lice limit matrix. Number of adult lice per farm that triggers treatment (l[,1]), spring treatment (l[,2]) and forced harvest (l[,3])
#' @param V Vector of seasons (winter, spring, summer) corresponding to each time-step in simulation
#' @param X Vector of survival (each life stage and genotype) after treatment X
#' @param Y Vector of survival (each life stage and genotype) after treatment Y
#' @param Zdisc Vector of survival (each life stage and genotype) after treatment Z (always discrete)
#' @param MAX Vector of survival after forced harvest
#' @param prod.cycle Matrix of farm production cycle - whether active (1) or inactive/fallow (0)
#' 
#'@return Populate N with the number of lice through time, save as data frame
#'
#'@param export

pop_2loci <- function(N, l, V, X, Y, Zdisc, MAX, prod.cycle) {
